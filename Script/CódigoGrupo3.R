...
EMPRESAS_FIL <- EMPRESAS_T %>% 
    select(Empresas = nombre_cia, Status = situacion, Tipo_de_empresa = tipo, Tamaño = tamanio,
           ...
           Activo = v499, Patrimonio = v698, Activo_no_Corriente = v498) %>% filter(Pasivo_corriente != 0, Activo != 0, Patrimonio != 0, Activo_no_Corriente != 0) %>% view("data_filtrada")
  
#Calculo de indicadores financieros  
EMPRESAS_FIL <- EMPRESAS_FIL %>% mutate(Liquidez_corriente = Activo_corriente/Pasivo_corriente) %>% #view() 
